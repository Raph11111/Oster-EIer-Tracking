...
from functools import lru_cache # Für Caching des öffentlichen Schlüssels

# NEUE IMPORTS FÜR KRYPTOGRAPHIE
...
# Globale Variable für den privaten Schlüssel (zum Signieren unserer eigenen Nachrichten)
private_key = None
# public_key_for_validation wird nicht mehr global für EINE Schlüssel verwendet,
# sondern dynamisch pro Nachricht abgerufen.

# --- Schlüssel-Ladefunktionen ---
    ...
    try:
        with open(file_path, "rb") as key_file:
            private_key = serialization.load_pem_private_key(
                key_file.read(),
                password=None, # Setze hier dein Passwort, falls vorhanden
...
# --- NEUE Funktion zum Laden/Cachen von öffentlichen Schlüsseln von URLs ---
@lru_cache(maxsize=128) # Cache bis zu 128 verschiedene öffentliche Schlüssel
def fetch_and_load_public_key(base_url):
    """
    Ruft einen öffentlichen Schlüssel von einer gegebenen Basis-URL ab und lädt ihn.
    Verwendet Caching, um wiederholte Abrufe zu vermeiden.
    """
    public_key_url = f"{base_url}/.well-known/egg_security.pub"
    print(f"Versuche öffentlichen Schlüssel von {public_key_url} abzurufen...")
    try:
        response = requests.get(public_key_url, timeout=5) # Timeout hinzufügen
        ...
        print(f"Öffentlicher Schlüssel von {base_url} erfolgreich geladen und gecacht.")
        return public_key
    except requests.exceptions.Timeout:
        print(f"Fehler: Timeout beim Abruf des öffentlichen Schlüssels von {public_key_url}")
        return None
    except requests.exceptions.RequestException as e:
        print(f"Fehler beim Abruf des öffentlichen Schlüssels von {public_key_url}: {e}")
        return None
    except Exception as e:
        print(f"Fehler beim Laden/Parsen des öffentlichen Schlüssels von {public_key_url}: {e}")
        return None


# --- Datenbank-Initialisierung (vor dem ersten Zugriff) ---
def init_db():
...
# --- Aktualisierte SNS-Sendefunktion mit Signierung ---
def sende_osterei_gefunden_nachricht(user_id, egg_id, base_url):
    ...
    if private_key is None:
        ...
        return # Oder eine Fehlermeldung an den Benutzer senden

    try:
        # Den zu signierenden String erstellen
        ...
        inner_sns_payload = {
            ...
            'url': base_url,
            'signature': signature_b64 # Die hinzugefügte Signatur
        }
        ...
        response = sns_client.publish(
            ...
            Message=json.dumps(sns_message_for_publish), # Sende das ÄUSSERE JSON
        ...
        print(f"Es gab einen unerwarteten Fehler beim Signieren/Senden an SNS: {e}")
...
def sns_notification():
    try:
        ...
        if message_type == 'SubscriptionConfirmation':
            ...
            # Die URL des Absenders, von der der öffentliche Schlüssel abgerufen werden kann
            sender_url = message.get('url') 
            signature_b64 = message.get('signature') # Die Signatur aus der Nachricht

            if user_id and egg_id and sender_url and signature_b64:
                print(f"Egg found - User: {user_id}, Egg ID: {egg_id}, Sender URL: {sender_url}")

                # --- Signaturvalidierung ---
                public_key_of_sender = fetch_and_load_public_key(sender_url)
                
                if public_key_of_sender is None:
                    print(f"Fehler: Konnte den öffentlichen Schlüssel von {sender_url} nicht laden. Signatur kann nicht validiert werden.")
                    return jsonify({'error': 'Could not load sender\'s public key'}), 500
                ...
                try:
                    ...
                    # Signatur mit dem ABGERUFENEN öffentlichen Schlüssel validieren
                    public_key_of_sender.verify( 
                        signature,
                    ...
                    print("Die Signatur ist UNGÜLTIG. Nachricht wird ignoriert.")
...
# --- ROUTE FÜR UNSEREN EIGENEN ÖFFENTLICHEN SCHLÜSSEL (aus .well-known) ---
@app.route('/.well-known/egg_security.pub')
def serve_public_key():
    public_key_path = os.path.join(app.root_path, 'static', '.well-known', 'egg_security.pub')
    if not os.path.exists(public_key_path):
        # Hier könntest du einen Fehler loggen oder eine generische Fehlermeldung zurückgeben
        print(f"Fehler: Öffentliche Schlüsseldatei nicht gefunden unter {public_key_path}")
